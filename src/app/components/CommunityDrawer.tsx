...
import { 
  X, HelpCircle, Rocket, UserPlus, FolderOpen,
...
interface CommunityDrawerProps {
  ...
  type: 'question' | 'progress' | 'teammate' | 'resource';
  projectId?: string;
  isDraft?: boolean;
}
...
export function CommunityDrawer({ 
  ...
  const typeConfig = {
    question: {
      title: '질문하기',
      description: '프로젝트 진행 중 막히는 부분을 물어보세요',
      icon: HelpCircle,
      color: 'blue',
    ...
    teammate: {
      title: '팀원 찾기',
      description: '함께 프로젝트를 진행할 팀원을 모집하세요',
      icon: UserPlus,
      color: 'green',
    ...
    <Drawer open={open} onOpenChange={onOpenChange} direction="right">
      <DrawerContent className="h-full w-full sm:max-w-2xl">
        <div className="h-full flex flex-col">
          {/* Header */}
          <DrawerHeader className="border-b border-gray-200 px-6 py-4 flex-shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <DrawerTitle className="text-xl">{config.title}</DrawerTitle>
                  <DrawerDescription className="text-sm">{config.description}</DrawerDescription>
                </div>
              </div>
              <DrawerClose asChild>
            ...
            <div className="space-y-6">
                ...
                <Card className="p-4 bg-gradient-to-br from-blue-50 to-purple-50 border-blue-200">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-gray-900 mb-1 text-sm">연결된 프로젝트</h3>
                      <p className="text-xs text-gray-600 mb-3">이 질문은 아래 프로젝트와 자동으로 연결됩니다</p>
                      
                      <div className="bg-white rounded-lg p-3 space-y-2">
                        <div>
                          <p className="text-xs text-gray-600 mb-1">프로젝트명</p>
                          <p className="font-semibold text-gray-900 text-sm">{projectData.title}</p>
                        </div>
                        ...
                        {projectData.type === 'contest' && projectData.contest && (
                          <div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <p className="text-xs font-semibold text-purple-600">{projectData.contest.name}</p>
                            </div>
                          </div>
                        )}
              ...
              {!projectId && isDraft && draftProjectData && (
                <Card className="p-4 bg-gradient-to-br from-blue-50 to-purple-50 border-blue-200">
                  <div className="flex items-start gap-3">
                    <Sparkles className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" />
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-gray-900 mb-1 text-sm">작성 중인 프로젝트</h3>
                      <p className="text-xs text-gray-600 mb-3">현재 생성 중인 프로젝트와 연결됩니다</p>
                      
                      <div className="bg-white rounded-lg p-3 space-y-2">
                        <div>
                          <p className="text-xs text-gray-600 mb-1">프로젝트명</p>
                          <p className="font-semibold text-gray-900 text-sm">{draftProjectData.title}</p>
                        </div>
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-blue-500 transition-colors cursor-pointer">
                  ...
                  <p className="text-xs text-gray-600 mb-0.5">이미지를 드래그하거나 클릭하여 업로드</p>
              ...
              {type === 'teammate' && (
                <Card className="p-4 bg-gray-50">
                  <h3 className="font-bold text-gray-900 mb-3 text-sm">모집 정보</h3>
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mb-2">
                        ...
                        />
                        <Button onClick={handleAddPosition} disabled={!positionInput.trim()} size="sm">
                          추가
                        </Button>
                      </div>
                      {formData.positions.length > 0 && (
                        <div className="flex flex-wrap gap-1.5">
                          {formData.positions.map((position) => (
                            <Badge
                              key={position}
                              className="bg-green-100 text-green-700 border-0 px-2 py-1 text-xs"
                            >
                              {position}
                              <button
              ...
              {type === 'resource' && (
                <Card className="p-4 bg-gray-50">
                  <h3 className="font-bold text-gray-900 mb-3 text-sm">자료 정보</h3>
                  <div className="space-y-3">
                    {/* 자료 URL */}
                    <div>
                      <label className="block text-sm font-semibold text-gray-900 mb-2">
                ...
                {formData.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {formData.tags.map((tag) => (
                      <Badge
                        key={tag}
                        className="bg-blue-100 text-blue-700 border-0 px-2 py-1 text-xs"
                      >
                        #{tag}
                        <button
